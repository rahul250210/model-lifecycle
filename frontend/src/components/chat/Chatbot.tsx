...
interface Message {
    ...
    data?: any[];
    query?: string;
    type?: 'text' | 'sql' | 'error' | 'comparison' | 'download' | 'factories';
    entity_type?: string;
    report_type?: string;
...
function DownloadReportButton({ reportType, reportName }: { reportType: string, reportName?: string | null }) {
    const { theme } = useTheme();
                                                ...
                                                <Paper elevation={0} sx={{
                                                    ...
                                                    background: msg.role === 'user'
                                                        ...
                                                        : msg.type === 'error'
                                                            ...
                                                            : mode === 'dark'
                                                                ? 'rgba(30, 41, 59, 0.45)' // Sleek Slate 800 Glass
                                                                : 'rgba(241, 245, 249, 0.85)', // Sleek Slate 100 Glass
                                                    border: msg.role === 'bot'
                                                    ...
                                                    <Typography variant="body2" component="div" sx={{
                                                        ...
                                                        <ReactMarkdown
                                                            components={{
                                                                h1: ({ node, ...props }) => <Typography variant="h6" fontWeight={850} sx={{ mt: 1.5, mb: 1, color: theme.textMain, fontSize: '1rem' }} {...props} />,
                                                                h2: ({ node, ...props }) => <Typography variant="subtitle2" fontWeight={800} sx={{ mt: 1.5, mb: 0.8, color: theme.textMain, fontSize: '0.9rem' }} {...props} />,
                                                                p: ({ node, ...props }) => <Typography variant="body2" sx={{ mb: 1, color: msg.role === 'user' ? '#fff' : theme.textMain, fontSize: '0.86rem', lineHeight: 1.6 }} {...props} />,
                                                                ...
                                                                code: ({ node, inline, ...props }: any) => inline ? (
                                                                    <Box component="code" sx={{
                                                                        ...
                                                                        color: msg.role === 'user' ? '#fff' : theme.primary, fontWeight: 700
                                                                    }} {...props} />
                                                                ) : (
                                                                ...
                                                                table: ({ node, ...props }) => (
                                                                    <Box sx={{ overflowX: 'auto', my: 2, borderRadius: '12px', border: `1px solid ${alpha(theme.border, 0.4)}`, boxShadow: '0 2px 8px rgba(0,0,0,0.02)' }}>
                                                                        <Box component="table" sx={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' }} {...props} />
                                                                    </Box>
                                                                ),
                                                                thead: ({ node, ...props }) => <Box component="thead" sx={{ bgcolor: alpha(theme.primary, 0.08) }} {...props} />,
                                                                tr: ({ node, ...props }) => <Box component="tr" sx={{ borderBottom: `1px solid ${alpha(theme.border, 0.2)}` }} {...props} />,
                                                                th: ({ node, ...props }) => <Box component="th" sx={{ px: 1.5, py: 1, textTransform: 'uppercase', letterSpacing: 0.8, fontWeight: 800, color: theme.textSecondary, textAlign: 'left' }} {...props} />,
                                                                td: ({ node, ...props }) => <Box component="td" sx={{ px: 1.5, py: 0.8, color: theme.textMain, fontWeight: 500 }} {...props} />,
                                                            }}
                                                        >
                                                            {msg.content}
                                                        </ReactMarkdown>
                                                    ...
                                                    {/* SQL Results Table / Entity Cards List */}
                                                    {msg.type === 'sql' && msg.data && msg.data.length > 0 && (() => {
                                                        const getEntityType = (m: Message) => {
                                                            ...
                                                            if (keys.includes('version_number') || keys.includes('accuracy') || keys.includes('f1_score')) {
                                                                return 'versions';
                                                            }
                                                            
                                                            // Inspect SQL query main table if available
                                                            ...
                                                            if (keys.includes('model_name') || queryLower.includes('models') || queryLower.includes('model')) {
                                                                return 'models';
                                                            }
                                                            if (keys.includes('factory_name') || queryLower.includes('factories') || queryLower.includes('factory')) {
                                                                return 'factories';
                                                            }
                                                            if (keys.includes('algorithm_name') || queryLower.includes('algorithms') || queryLower.includes('algorithm')) {
                                                                return 'algorithms';
                                                            }
                                                            if (keys.includes('model_name') || (keys.includes('name') && queryLower.includes('models'))) {
                                                                return 'models';
                                                            }
                                                            return null;
                                                        };

                                                        const entityType = getEntityType(msg);
                                                        const isEntityList = entityType && (msg.data[0].name !== undefined || msg.data[0].version_number !== undefined);

                                                        if (isEntityList) {
                                                            return <EntityList data={msg.data} type={entityType as any} />;
                                                        }

                                                        // Otherwise render default SQL table, but strip all ID columns
                                                        const isIdColumn = (col: string) => {
                                                            const lower = col.toLowerCase();
                                                            return lower === 'id' || lower.endsWith('_id');
                                                        };
                                                        const cols = Object.keys(msg.data[0]).filter(col => !isIdColumn(col));
                                                        ...
                                                        return (
                                                            <Box sx={{
                                                                ...
                                                                boxShadow: `0 4px 16px -4px ${alpha('#000', 0.06)}`,
                                                            }}>
                                                                <Box sx={{ overflowX: 'auto' }}>
                                                                    <Box component="table" sx={{
                                                                        ...
                                                                        <Box component="thead">
                                                                            <Box component="tr" sx={{
                                                                                background: `linear-gradient(135deg, ${alpha(theme.primary, 0.12)}, ${alpha(theme.primary, 0.06)})`,
                                                                            ...
                                                                            {msg.data.map((row: any, rIdx: number) => (
                                                                                <Box
                                                                                    ...
                                                                                    sx={{
                                                                                        bgcolor: rIdx % 2 === 0
                                                                                            ? 'transparent'
                                                                                            : alpha(theme.textMain, 0.025),
                                                                                        borderBottom: rIdx < msg.data!.length - 1
                                                                                            ? `1px solid ${alpha(theme.border, 0.2)}`
                                                                                            : 'none',
                                                                                    ...
                                                                                    {cols.map(col => (
                                                                                        ...
                                                                                        >
                                                                                            {row[col] !== null && row[col] !== undefined
                                                                                                ? String(row[col])
                                                                                                : <Box component="span" sx={{ color: theme.textMuted, fontStyle: 'italic' }}>—</Box>
                                                                                            }
                                                                                        </Box>
                                                                                    ))}
                                                                                </Box>
                                                    ...
                                                    {msg.type === 'comparison' && msg.data && msg.data.length >= 2 && (
                                                        <ComparisonButton
                                                            versions={msg.data}
                                                            onClick={() => setComparisonModal({ open: true, data: msg.data! })}
                                                        />
                                                    )}
